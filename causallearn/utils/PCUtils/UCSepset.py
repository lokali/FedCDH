...
def uc_sepset(cg: CausalGraph, priority: int = 4,
              background_knowledge: BackgroundKnowledge | None = None, cg_list: List[CausalGraph] | None = None, K: int=0) -> CausalGraph:
    """
    ...
    # print(f"102 This is it: {priority}")
    ...
    if priority in [0, 1, 2]:
        ...
        if priority == 3:  # 3. Order colliders by p_{xz|y} in ascending order
            # print(f"103 This is it: {priority}")
            print(f"102: In UCSepset: {priority}, flag={flag}")
            for (x, y, z) in R0:
                cond = cg_new.find_cond_sets_with_mid(x, z, y)

                p_list = []
                for S in cond:
                    p_ = cg_new.ci_test(x, z, S) 
                    p_list.append(p_)
                UC_dict[(x, y, z)] = max(p_list)
            ...
            """
            Flag: 
                0: Centralized setting of origianal CD-NOD; 
                1: Federated learning setting using Max p-value; 
                2: Directly get CIT results from previously saved UCsepset.
            """
            flag = 2
            # print(f"In Orientation UCSepset: {priority}, Flag={flag}. UCsepset length: {len(R0)}")
            for (x, y, z) in R0:
                ...
                #         p_list.append(p_) 
                #     elif flag == 1:
                #         p_ = [cg_list[k].ci_test(x, z, S) for k in range(K)]
                #         p = max(p_)
                #         p_list.append(p)                    
                #     elif flag == 2:
                #         p_ = [cg_list[k].ci_test(x, y, S) for k in range(K)]
                #         p = sum(p_)/len(p_)
                #         p_list.append(p)
                #     elif flag == 3:
                #         p_ = [cg_list[k].ci_test(x, y, S) for k in range(K)]
                ...
                if flag==0:
                    ...
                    # print("Hi, I am in UCsepset.")
                    p_val = 0
                    sepset = cg_new.sepset[x,z]
