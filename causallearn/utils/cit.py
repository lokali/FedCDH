...
from causallearn.utils.KCI.KCI import KCI_CInd, KCI_UInd, GMM_CInd, GMM_UInd, LGM_UInd, LGM_CInd_1, LGM_CInd_2
...
class KCI(CIT_Base):
    ...
    def __call__(self, X, Y, condition_set=None, gmm=0, K=0):
        ...
        if gmm==0: # normal KCI
            ...
            p = self.gmm_ui.compute_pval(self.data[:, Xs], self.data[:, Ys], K) if len(condition_set) == 0 else \
                self.gmm_ci.compute_pval(self.data[:, Xs], self.data[:, Ys], self.data[:, condition_set], K) 
            ...
            if len(condition_set)==0:
                ...
                p = self.lgm_ci_1.compute_pval(self.data[:, Xs], self.data[:, Ys], self.data[:, condition_set], K)
            else:
                p = self.lgm_ci_2.compute_pval(self.data[:, Xs], self.data[:, Ys], self.data[:, condition_set], K)
            ...
            if len(condition_set)==0:
                ...
                p = self.kci_ci.compute_pvalue_rf(self.data[:, Xs], self.data[:, Ys], self.data[:, condition_set])[0]
